...
import SessionSlideOver from '@/Components/SessionSlideOver.vue'
...
function openSlide(topic: any) {
    slideTopic.value = topic
    slideVisible.value = true
}

function closeSlide() {
    slideVisible.value = false
    slideTopic.value = null
}

function toggleComplete(topicId: number) {
        ...
        <template #header>
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center gap-3">
                    <span class="badge" :class="plan.level === 'beginner' ? 'badge-green' : plan.level === 'intermediate' ? 'badge-yellow' : 'badge-red'">
        ...
        <div class="space-y-8">
            <div :class="['rounded-xl border border-gray-200/60 bg-white/60 p-6 shadow-sm dark:border-gray-800/40 dark:bg-gray-900/40', 'transition-all duration-700 ease-out-expo', mounted ? 'translate-y-0 opacity-100' : 'translate-y-6 opacity-0']">
                ...
                <div class="mt-4 h-2 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
                    <div class="h-full rounded-full bg-gradient-to-r from-brand-500 to-brand-600 transition-all duration-1000 ease-[cubic-bezier(0.16,1,0.3,1)]" :style="{ width: progressPercent + '%' }" />
                </div>

                <div v-if="plan.weeks.length > 1" class="mt-5 flex flex-wrap gap-2">
                    <button v-for="week in plan.weeks" :key="week.id" @click="scrollToWeek(week.id)" class="flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 transition-all hover:border-brand-200 hover:text-brand-600 dark:border-gray-700 dark:text-gray-400 dark:hover:border-brand-800 dark:hover:text-brand-400">
            ...
            <div v-if="plan.status === 'failed'" class="rounded-xl border border-red-200/60 bg-red-50/60 p-6 dark:border-red-900/30 dark:bg-red-950/20">
                <div class="flex items-center justify-between">
                    ...
                    <button @click="router.post(route('study-plans.retry', plan.id))" class="btn-primary bg-red-600 hover:bg-red-700 px-5 py-2 text-sm">
            ...
            <div :class="['space-y-4', 'transition-all duration-700 delay-150 ease-out-expo', mounted ? 'translate-y-0 opacity-100' : 'translate-y-6 opacity-0']">
                ...
                >
                    <button @click="toggleWeek(week.id)" class="flex w-full items-center justify-between p-6 text-left transition-all duration-200 hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
                        <div class="flex items-center gap-4">
                            <div :class="[
                                'flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-sm font-bold shadow-sm transition-all duration-300',
                                completedTopicsCount(week) === week.topics.length ? 'bg-emerald-500 text-white' : 'bg-brand-600 text-white'
                            ]">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="hidden h-1.5 w-20 overflow-hidden rounded-full bg-gray-100 sm:block dark:bg-gray-800">
                                <div class="h-full rounded-full bg-gradient-to-r from-brand-500 to-brand-600 transition-all duration-500" :style="{ width: week.topics.length > 0 ? (completedTopicsCount(week) / week.topics.length) * 100 + '%' : '0%' }" />
                            </div>
                            <Icon :name="activeWeek === week.id ? 'chevron-up' : 'chevron-down'" :size="18" class="text-gray-400 transition-all duration-300" />
                        </div>
                    </button>

                    <transition name="accordion">
                        <div v-if="activeWeek === week.id" class="border-t border-gray-100 dark:border-gray-800/50">
                            <div v-for="(topic, index) in week.topics" :key="topic.id" :class="[
                                'border-b border-gray-50 transition-all last:border-0 dark:border-gray-800/30',
                                topic.is_completed ? 'opacity-60' : ''
                            ]">
                                <button @click="openSlide(topic)" class="flex w-full items-center gap-4 p-4 text-left transition-all hover:bg-gray-50/50 dark:hover:bg-gray-800/20">
                                    <div @click.stop="toggleComplete(topic.id)" :class="[
                                        'flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 transition-all duration-200',
                                        topic.is_completed
                                            ? 'border-emerald-500 bg-emerald-500'
                                            : 'border-gray-300 hover:border-brand-400 dark:border-gray-600 dark:hover:border-brand-500'
                                    ]">
                                        <Icon v-if="topic.is_completed" name="check" :size="11" class="text-white" />
                                    </div>
                                    ...
                                    <div class="flex min-w-0 flex-1 items-center gap-3">
                                        <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400">
                                            <Icon :name="contentTypeIcons[topic.content_type] || 'file-text'" :size="14" />
                                        </div>
                                        <div class="min-w-0 flex-1">
                                            <h4 :class="[
                                                'text-sm font-medium transition-all',
                                                topic.is_completed ? 'text-gray-400 line-through dark:text-gray-500' : 'text-gray-900 dark:text-white'
                                            ]">{{ topic.title }}</h4>
                                            <div class="flex items-center gap-2 text-xs text-gray-400 dark:text-gray-500">
                                                <span>{{ topic.estimated_minutes }} min</span>
                                                <span v-if="topic.sessions?.length">&middot; {{ topic.sessions.length }} sesiones</span>
                                                <span v-if="topic.is_completed && topic.completed_at">&middot; Completado</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex shrink-0 items-center gap-2">
                                        <span :class="['badge', difficultyColors[topic.difficulty] || 'badge-gray']">
                                            ...
                                            {{ topic.difficulty === 'beginner' ? 'Basico' : topic.difficulty === 'intermediate' ? 'Intermedio' : 'Avanzado' }}
                                        </span>
                                        <div class="flex h-8 w-8 items-center justify-center rounded-lg text-gray-300 transition-all group-hover:text-brand-500 dark:text-gray-600">
                                            <Icon name="chevron-right" :size="16" />
                                        </div>
                                    </div>
                                </button>

                                <div v-if="topic.sessions?.length" class="flex flex-wrap items-center gap-1.5 px-4 pb-4">
                                    <div v-for="session in topic.sessions" :key="session.id" class="group flex items-center gap-1.5 rounded-lg bg-gray-50/50 px-2.5 py-1 text-xs dark:bg-gray-800/30">
                                        <Icon name="clock" :size="10" class="text-gray-400" />
                                        <span class="font-medium text-gray-600 dark:text-gray-400">{{ formatMinutes(session.duration_minutes) }}</span>
                                        <span class="text-gray-400">|</span>
                                        <span class="flex items-center gap-0.5 text-gray-500">
                                            <Icon name="star" :size="9" />
                                            {{ session.confidence_level }}/5
                                        </span>
                                        <button @click="deleteSession(session.id)" class="ml-1 text-gray-300 opacity-0 transition-all hover:text-red-500 group-hover:opacity-100 dark:text-gray-600">
                                            <Icon name="x" :size="10" />
                                        </button>
                                    </div>
                                </div>
